...
const OpenAppLogo: FC<Partial<OpenApp>> = ({ shortName }) => {
  switch (shortName) {
    case 'zapier':
      return (
        <IconBrandZapier className="w-12 h-12 p-3 text-[#ff4f00] bg-white shadow rounded-full bg-white" />
      )
...
export const OpenAppAuthorize = (): JSX.Element => {
  ...
  return (
    <AsyncRequest
      ...
      immediate={!!clientId}
      skeleton={<Loading />}
      emptyState={<EmptyStates icon={<IconApiApp />} title="App not found" />}
    >
      <div className="mb-12">
      ...
      <div className="space-y-5">
        <div>
          ...
          <div className="text-sm text-slate-700">
            {t('openApp.authorize.account.description', { openAppName: openApp?.name })}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="text-sm text-slate-700">
            {t('openApp.authorize.product.description', { openAppName: openApp?.name })}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="text-sm text-slate-700">
            {t('openApp.authorize.contact.description', { openAppName: openApp?.name })}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="text-sm text-slate-700">
            {t('openApp.authorize.payment.description', { openAppName: openApp?.name })}
          </div>
        </div>
      </div>
      ...
      <div className="mt-10 space-y-2">
        ...
        <Button.Link className="block w-full !py-2" onClick={handleCancel}>
          Cancel
        </Button.Link>
      </div>
    </AsyncRequest>
